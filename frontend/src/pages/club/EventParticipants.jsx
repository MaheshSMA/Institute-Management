...
function EventParticipants() {
  ...
  const save = async (p) => {
    ...
    const finalStatus =
      data.Participation_status ?? p.Participation_status;

    const finalPts =
      Number(data.Pts_earned ?? p.Pts_earned) || 0;
    ...
    /* ✅ SHOW TOAST ONLY IF COMPLETED + POINTS > 0 */
    if (finalStatus === "Completed" && finalPts > 0) {
      setToast(
        `${finalPts} activity points added to ${p.Student_name}`
      );
  ...
  /* ---------------- CSV EXPORT ---------------- */
  const exportCSV = () => {
    const headers = ["Student Name", "Status", "Role", "Points"];
