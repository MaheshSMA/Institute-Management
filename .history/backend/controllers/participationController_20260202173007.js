...
const updateParticipation = async (req, res) => {
  ...
  try {
    ...
    await db.query(
      ...
      `,
      [status, role, earnedPts, eventId, studentId]
    );

    /* ---------- 3️⃣ ADD points to STUDENT (NO SUBTRACTION EVER) ---------- */
    if (status === "Completed" && earnedPts > 0) {
      await db.query(
        `
        UPDATE STUDENT
