...
// naacController.js
const uploadNAACEvidence = async (req, res) => {
  try {
    ...
    await db.query(
      `INSERT INTO NAAC_EVIDENCE
       (Metric_code, Title, File_path, Uploaded_by)
       VALUES (?, ?, ?, ?)`,
      [
        metricCode,
...
const getMetricEvidence = async (req, res) => {
  const { metricCode } = req.params;

  const [rows] = await db.query(
    `SELECT Evidence_id, Title, File_path, Uploaded_by, Uploaded_at
     FROM NAAC_EVIDENCE
     WHERE Metric_code = ?
