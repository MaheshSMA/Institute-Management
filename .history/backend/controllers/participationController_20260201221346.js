...
const updateParticipation = async (req, res) => {
  ...
  try {
    // 1️⃣ Get current participation status
    ...
    // 3️⃣ Add activity points ONLY when:
    // previous status != Completed AND new status == Completed
    if (
      current.Participation_status !== "Completed" &&
      status === "Completed"
    ) {
      // inside updateParticipation, BEFORE updating
    const [[event]] = await db.query(
      `SELECT Event_date FROM CLUB_EVENT WHERE Event_id = ?`,
      [eventId]
    );
    ...
    if (event.Event_date < today) {
      ...
      if (event) {
        await db.query(
          ...
          WHERE Student_id = ?
          `,
          [event.Activity_points, studentId]
        );
      }
    }
