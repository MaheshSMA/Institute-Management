...
const updateParticipation = async (req, res) => {
  ...
  try {
    /* ---------- 1️⃣ Fetch current participation ---------- */
    ...
    /* ---------- 3️⃣ Revert old points if already completed ---------- */
    if (current.Participation_status === "Completed") {
      await db.query(
        ...
        WHERE Student_id = ?
        `,
        [current.Pts_earned || 0, studentId]
      );
    }

    /* ---------- 4️⃣ Update participation row ---------- */
    await db.query(
      `
      UPDATE PARTICIPATION
      SET
    ...
    /* ---------- 5️⃣ Apply new points if completed ---------- */
    if (status === "Completed" && newPts > 0) {
      await db.query(
        `
        UPDATE STUDENT
        SET Activity_pts = COALESCE(Activity_pts, 0) + ?
