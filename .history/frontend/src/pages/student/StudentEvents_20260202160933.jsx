      ...
      {events.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {events.map((e) => (
            <div
              key={e.Event_id}
              className="bg-white border rounded-2xl shadow-sm p-6 flex flex-col justify-between hover:shadow-md transition"
            >
              <div>
              ...
              {e.isRegistered ? (
                <span className="mt-4 inline-block text-center px-3 py-2 bg-green-100 text-green-700 rounded-full text-sm font-medium">
                  ✔ Registered
                </span>
              ) : (
                <button
                  disabled={submittingId === e.Event_id}
                  onClick={() => participate(e.Event_id)}
                  className={`mt-4 py-2 rounded-md font-medium transition
                    ${
                      submittingId === e.Event_id
