...
function FacultyDashboard() {
  ...
  // Simple dashboard stats (can be wired to real APIs later)
  ...
  useEffect(() => {
    ...
    API.get("/clubs")
      .then((res) => {
        const myClubs = res.data.filter(
          (club) => club.Coordinator_id === facId
        );
        setIsCoordinator(myClubs.length > 0);
    ...
    // Optional: fetch request stats (safe fallback if API not present)
