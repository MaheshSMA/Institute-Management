...
function FacultyAIInsights() {
  ...
  try {
    ...
    if (selected) {
      ...
      if (updated) {
        ...
        // 🟢 STOP polling once AI is ready
        if (
          updated.ai_source &&
          updated.ai_source.toLowerCase() === "ollama"
        ) {
          setPolling(false);
        }
      }
  ...
  <div className="flex flex-wrap gap-1">
    {normalizeTopics(ins.dominant_topics).map((t, i) => (
      <span
        key={i}
        ...
        {t}
                  ...
                  <td className="px-4 py-3 text-sm text-gray-500">
                    <div className="flex flex-col gap-1">
                      ...
                      {aiBadge(
                        typeof ins.ai_source === "string"
                          ? ins.ai_source.trim().toLowerCase()
                          : ""
                      )}
                    </div>
                  </td>
      ...
      {selected && (
        <div className="bg-white border rounded-xl shadow-sm p-6 mt-6">
          ...
          <button
            onClick={async () => {
              await API.post(`/ai/regenerate/${selected.Student_id}`);
