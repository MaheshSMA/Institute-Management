...
def generate_insight(student_id):
    ...
    # 🔒 Skip if AI already generated in last 10 minutes
    cur.execute("""
        SELECT generated_at
        FROM STUDENT_AI_INSIGHTS
        WHERE Student_id = %s
          AND generated_at > NOW() - INTERVAL 10 MINUTE
    """, (student_id,))

    if cur.fetchone():
        print(f"⏭ Skipping AI run for student {student_id} (recent insight exists)")
        conn.close()
        return
