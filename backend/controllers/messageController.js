...
const getClubMessagesWithStudent = async (req, res) => {
  const clubId = req.user.refId;
  ...
  const [rows] = await db.query(`
    ...
    FROM MESSAGE
    WHERE
      (Sender='Student' AND Sender_id=? AND Receiver_id=?)
      OR
      (Sender='Club' AND Sender_id=? AND Receiver_id=?)
    ORDER BY Created_At ASC
  `, [studentId, clubId, clubId, studentId]);
...
const sendClubMessage = async (req, res) => {
  ...
  await db.query(`
    INSERT INTO MESSAGE
    (Sender, Sender_id, Receiver_id, Content)
    VALUES ('Club', ?, ?, ?)
  `, [clubId, studentId, content]);
...
module.exports = {
  getMessages,
  sendMessage,
  getClubConversations,
  getClubMessagesWithStudent,
  sendClubMessage
};
